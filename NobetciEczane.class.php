    ...
    private function parcala(){
        ...
        for($i=0;$i<count($basliklar[1]);$i++) {
            ...
            if(($seo_baslik != null) && ($seo_baslik != 'reklam') && ($seo_baslik != 'google-reklam')){
                $this->verilerArray[$y]["eczaneAdi"] = strip_tags(trim($basliklar[1][$i]));
                $bol   = explode("<br>",$detaylar[1][$y]);
                $bol_2 = explode('<br />',$bol[0]);

                $this->verilerArray[$y]["eczaneAdres"] = strip_tags(trim($bol_2[1]));
                $this->verilerArray[$y]["eczaneTelefon"] = strip_tags(trim($bol_2[4]));
                $y++;
            }
